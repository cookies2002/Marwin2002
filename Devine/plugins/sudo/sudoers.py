...
async def useradd(client, message: Message):
    ...
    if user.id in SUDOERS:
        return await message.reply_text(f"{user.mention} ɪs ᴀʟʀᴇᴀᴅʏ ᴀᴜᴛʜᴏʀɪᴢᴇᴅ ᴀs ᴀ ᴡᴀʀʟᴏʀᴅ.")

    added = await add_sudo(user.id)
    if added:
        SUDOERS.add(user.id)
        await message.reply_text(f"{user.mention} ʜᴀs ʙᴇᴇɴ ᴀᴜᴛʜᴏʀɪᴢᴇᴅ ᴀs ᴀ ᴡᴀʀʟᴏʀᴅ.")
        
        await log_new_sudo_user(user, message.from_user, message.chat)
        
    else:
        await message.reply_text("ᴛʜᴇ ᴀᴛᴛᴇᴍᴘᴛ ᴛᴏ ᴀᴅᴅ ᴛʜᴇ ᴡᴀʀʟᴏʀᴅ ᴜsᴇʀ ᴡᴀs ᴜɴsᴜᴄᴄᴇssғᴜʟ. ᴘʟᴇᴀsᴇ ᴀᴛᴛᴇᴍᴘᴛ ᴀɢᴀɪɴ.")
...
async def userdel(client, message: Message):
    ...
    if user.id not in SUDOERS:
        return await message.reply_text(f"{user.mention} ɪs ɴᴏᴛ ᴀᴜᴛʜᴏʀɪᴢᴇᴅ ᴛᴏ ᴜsᴇ ᴡᴀʀʟᴏʀᴅ ᴘᴏᴡᴇʀ.")

    removed = await remove_sudo(user.id)
    if removed:
        SUDOERS.remove(user.id)
        await message.reply_text(f"ᴡᴀʀʟᴏʀᴅ ᴘᴏᴡᴇʀ  ғᴏʀ {user.mention} ʜᴀs ʙᴇᴇɴ ʀᴇᴠᴏᴋᴇᴅ.")
        
        await log_removed_sudo_user(user, message.from_user, message.chat)
        
    else:
        await message.reply_text("ᴛʜᴇ ᴀᴛᴛᴇᴍᴘᴛ ᴛᴏ ʀᴇᴍᴏᴠᴇ ᴛʜᴇ ᴡᴀʀʟᴏʀᴅ ᴜsᴇʀ ᴡᴀs ᴜɴsᴜᴄᴄᴇssғᴜʟ. ᴘʟᴇᴀsᴇ ᴀᴛᴛᴇᴍᴘᴛ ᴀɢᴀɪɴ.")



@app.on_message(filters.command(["sudolist", "warlords", "suser"]) & ~BANNED_USERS)
...
async def welcome_special_users(client, update: ChatMemberUpdated):
    ...
    if new_chat_member.status == ChatMemberStatus.MEMBER:
        ...
        if user_id == OWNER_ID:
            ...
            message = f"❄️ ᴡᴀʀʟᴏʀᴅ ɪs ɪɴ ᴛʜᴇ ᴄʜᴀᴛ ɴᴏᴡ."
        else:
