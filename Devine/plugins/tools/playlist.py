...
from Devine.utils import close_markup
from config import filter as BANNED_USERS, SERVER_PLAYLIST_LIMIT
from Devine import Carbon, app
from Devine.utils.decorators.language import language, languageCB
from Devine.utils.inline.playlist import (
    botplaylist_markup,
    get_playlist_markup,
    warning_markup,
)
from Devine.utils.pastebin import devineBin
import time
import asyncio
import yt_dlp
...
async def add_song(client, message: Message, _):
    ...
    adding = await message.reply_text("🎧 Searching for the song, please wait..")
    ...
    try:
        ...
        if 'items' not in data or not data['items']:
            return await message.reply_text("**➻ No song found with the provided name.**")
        ...
        await adding.delete()
        return await message.reply_text(
            text="**➻ Song has been added successfully from your search✅**\n\n**➥ If you want to remove any song then click the button below.**\n\n▷ Check by » /showplaylist\n\n▷ **Play by » /play**"
        )

    except Exception as e:
        return await message.reply_text(f"**Error searching for song:** {str(e)}")
