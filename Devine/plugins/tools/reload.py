...
from config import filter, adminlist, lyrical
...
@app.on_message(
    filters.command(["admincache", "reload", "refresh"], prefixes=["/", "!", "%", ",", "", ".", "@", "#"]) & filters.group & ~filter
)
@language
...
@app.on_message(filters.command(["reboot"]) & filters.group & ~filter)
@AdminActual
async def restartbot(client, message: Message, _):
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await devine.stop_stream_force(chat_id)
        except:
            pass
             ...
             [
                 [
                      InlineKeyboardButton(
                         "• 𝖧ᴀᴄᴋᴇᴅ вʏ  •", url=f"https://t.me/Legend_mickey")
                 ]
            ]
         ),
     )
...
@app.on_callback_query(filters.regex("close") & ~filter)
...
@app.on_callback_query(filters.regex("stop_downloading") & ~filter)
@ActualAdminCB
